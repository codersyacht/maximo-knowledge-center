...
**Note:** Replace codehub.dev.fyre.ibm.com with appropriate hostname.

Without TLS:
```CMD
mongosh --host codehub.dev.fyre.ibm.com --port 27017 admin
```
With TLS:
```CMD
...
Without TLS:
```CMD
mongosh --host codehub.dev.fyre.ibm.com --port 27017  -u "admin" -p "password" admin
```
With TLS:
```CMD
